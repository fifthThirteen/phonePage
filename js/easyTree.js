        ...
        this.each(function () {
            ...
            $.each($(easyTree).find('ul > li'), function() {
                ...
                if($(this).is('li:has(ul)')) {
                    ...
                    $(children).remove();
                    text = $(this).text();
                    $(this).html('<span><span class="glyphicon"></span><label id="name" class="name"></label></span><a href="#myModal" class="glyphicon glyphicon-search" data-toggle="modal" data-target="#myModal"></a>');
                    $(this).find(' > span > span').addClass('glyphicon-minus');
                    $(this).find(' > span > label').text(text);
            ...
            if (options.addable) {
                ...
                $(easyTree).find('.easy-tree-toolbar .create > button').attr('title', options.i18n.addTip).click(function () {
                    ...
                    $(createInput).find('.confirm').click(function () {
                        ...
                        var item = $('<li><span><span class="glyphicon glyphicon-user"></span><a href="javascript: void(0);">' + $(createInput).find('input').val() + '</a> </span></li>');
                        $(item).find(' > span > span').attr('title', options.i18n.collapseTip);
                        $(item).find(' > span > label').attr('title', options.i18n.selectTip);
                        if (selected.length <= 0) {
                            ...
                            if ($(selected).hasClass('parent_li')) {
                                ...
                                $(selected).addClass('parent_li').find(' > span > span').addClass('glyphicon-folder-open').removeClass('glyphicon-user');
                        ...
                        if (options.selectable) {
                            $(item).find(' > span > label').attr('title', options.i18n.selectTip);
                            $(item).find(' > span > label').click(function (e) {
            ...
            if (options.editable) {
                ...
                $(easyTree).find('.easy-tree-toolbar .edit > button').attr('title', options.i18n.editTip).click(function () {
                    ...
                    $(easyTree).find('li > span > label:hidden').show();
                    var selected = getSelectedItems();
                    if (selected.length <= 0) {
                        $(easyTree).prepend(warningAlert);
                        ...
                        var value = $(selected).find(' > span > label').text();
                        $(selected).find(' > span > label').hide();
                        ...
                        $(editor).keydown(function (e) {
                            if (e.which == 13) {
                                if ($(editor).val() !== '') {
                                    $(selected).find(' > span > label').text($(editor).val());
                                    $(editor).remove();
                                    $(selected).find(' > span > label').show();
                                }
                            }
                        });
                    }
                    ...
                    if (selected.length <= 0) {
                        ...
                        $(easyTree).find('.alert .alert-content .confirm').on('click', function () {
                            ...
                            if($(selected).parent('ul').find(' > li').length <= 1) {
                                $(selected).parents('li').removeClass('parent_li').find(' > span > span').removeClass('glyphicon-folder-open').addClass('glyphicon-user');
            ...
            if (options.selectable) {
                $(easyTree).find('li > span > label').attr('title', options.i18n.selectTip);
                $(easyTree).find('li > span > label').click(function (e) {
                    var li = $(this).parent().parent();
                    if (li.hasClass('li_selected')) {
                        $(this).attr('title', options.i18n.selectTip);
                        $(li).removeClass('li_selected');
